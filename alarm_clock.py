...
from PyQt6.QtCore import QEvent, QTime, QTimer, QUrl, Qt, QStandardPaths
from PyQt6.QtGui import QAction, QColor, QFont, QIcon, QKeySequence, QShortcut, QIntValidator
...
def calculate_next_trigger(alarm):
    ...
    if alarm["repeat_type"] == "none" or alarm["repeat_type"] == "daily":
        if candidate.replace(microsecond=0) < now.replace(microsecond=0):
...
class AlarmDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        btn_browse = QPushButton("Add Custom Audio")
        btn_browse.clicked.connect(lambda: self.add_custom_audio(self.combo_sound))
    ...
    def populate(self):
        ...
        self.combo_sound.setCurrentText(self.alarm.get("sound", "timer.mp3"))
    ...
    def save(self):
        ...
        if selected_sound != common_default and self.config.config["use_common_sound"]:
            reply = QMessageBox.question(
                self,
                "Disable Common Sound",
                "You selected an individual sound different from the common sound. Do you want to disable 'Use Common Default Sound'?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.Yes
            )
            if reply == QMessageBox.StandardButton.Yes:
                self.config.config["use_common_sound"] = False
...
class RingDialog(QDialog):
    ...
    def __init__(self, alarm, audio, config, parent=None):
        ...
        snd = self.alarm.get("sound", "timer.mp3")
...
class MainWindow(QMainWindow):
    ...
    def populate_audio_dropdown(self, combo):
        ...
        common_default = self.config_manager.config.get("common_sound_file", "timer.mp3")
        if combo.findText(common_default) >= 0:
            combo.setCurrentText(common_default)
        elif combo.findText("timer.mp3") >= 0:
            combo.setCurrentText("timer.mp3")
    ...
    def setup_ui(self):
        ...
        self.combo_common.setCurrentText(self.config_manager.config.get("common_sound_file", "timer.mp3"))

        btn_com_browse = QPushButton("Add Custom Audio")
        btn_com_browse.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        btn_com_browse.clicked.connect(lambda: self.add_custom_audio(self.combo_common))
    ...
    def save_globals(self):
        ...
        self.config_manager.config["use_common_sound"] = use_common
        self.config_manager.config["common_sound_file"] = common_snd

        if use_common:
            for alarm in self.config_manager.config["alarms"]:
                alarm["sound"] = common_snd
    ...
    def check_alarms(self):
        ...
        for alarm in self.config_manager.config["alarms"]:
            if alarm["active"] and alarm.get("next_trigger", ""):
                trigger_dt = datetime.datetime.fromisoformat(alarm["next_trigger"])
                if trigger_dt <= now:
